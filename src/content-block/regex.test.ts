...
describe("regex", () => {
  ...
  test("matches an embed without an internal content path", () => {
    const input = "{{embed:content_block_pension:1690ab79-1880-461e-99e4-ed146fd9efab}}";
  ...
  test("matches an embed with an internal content path", () => {
    const input = "{{embed:content_block_pension:1690ab79-1880-461e-99e4-ed146fd9efab/rates/rate-1/amount}}";
    const result = regex.exec(input);
  ...
  test("matches an embed with a format specifier", () => {
    const input = "{{embed:content_block_pension:1690ab79-1880-461e-99e4-ed146fd9efab#some_format}}";
    const result = regex.exec(input);
  ...
  test("does not match an unsupported document type", () => {
    const input = "{{embed:content_block_unknown:1690ab79-1880-461e-99e4-ed146fd9efab}}";
  ...
  test("finds multiple embeds in a single string", () => {
    const input = "Hello {{embed:contact:abc-123}} and {{embed:content_block_tax:def-456}}";
    ...
    expect(matches).toEqual(["{{embed:contact:abc-123}}", "{{embed:content_block_tax:def-456}}"]);
  });

  test("matches an embed surrounded by other text", () => {
    const input = "Some text {{embed:content_block_pension:1690ab79-1880-461e-99e4-ed146fd9efab}} more text";
    const result = regex.exec(input);

    expect(result).not.toBeNull();
    expect(result![0]).toEqual("{{embed:content_block_pension:1690ab79-1880-461e-99e4-ed146fd9efab}}");
  });

  test("validates full embed codes only", () => {
    expect(isValidEmbedCode("{{embed:content_block_pension:1690ab79-1880-461e-99e4-ed146fd9efab}}")).toBe(true);

    expect(isValidEmbedCode("prefix {{embed:contact:abc-123}} suffix")).toBe(false);
