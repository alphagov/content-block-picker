...
However, embed codes remain fully editable as regular text. Each character in an embed code like `{{embed:content_block_pension:new-state-pension}}` can be individually added, deleted, or modified.
...
4. **Current partial solution is incomplete**: The `adjustInsertPositionIfSelectionOverlapsEmbedCode()` method ([here](https://github.com/alphagov/content-block-picker/blob/CM-951-adr-to-treat-embed-codes-as-single-characters/src/content-block-picker.ts#L214)) already demonstrates awareness of this problem—it prevents new embed codes from being inserted into existing ones. However, this only addresses insertion, not editing.
...
Switch from `<textarea>` to a `contenteditable` element, representing embed codes as non-editable nested elements (e.g., `<span contenteditable="false">`). [MDN: contenteditable HTML global attribute](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Global_attributes/contenteditable)
