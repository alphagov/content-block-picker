...
var N = Ke(), qe = {
	...
	fetchPreview(e) {
		...
		let n = fetch(t).then(async (t) => t.ok ? {
			html: await t.text(),
			valid: !0
		} : (this.cache.delete(e), this.logAndPromiseError(`Failed to fetch block ${e} (${t.status})`))).catch((t) => (this.cache.delete(e), this.logAndPromiseError(`Error fetching block ${e}: ${t instanceof Error ? t.message : String(t)}`)));
	...
	updateHighlight() {
		...
		n[n.length - 1] === "\n" && (n += " "), n = n.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
		let r = Array.from(n.matchAll(t)), i = r.map((e) => this.apiClient.fetchPreview(e[0]).catch((t) => (console.warn(`Failed to pre-fetch preview for ${e[0]}`, t), {
			html: "",
			valid: !1
		})));
